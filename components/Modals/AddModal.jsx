...
const AddModal = () => {
    const { addToggle, setAddToggle, user, setCount, mode } = useContext(AppContext);
        ...
        <div>
            ...
            <div id="authentication-modal" tabIndex="-1" aria-hidden="true" className={`${!addToggle && "hidden"} overflow-y-auto overflow-x-hidden fixed top-0 right-0 left-0 z-50 justify-center items-center w-full md:inset-0 h-[calc(100%-1rem)] max-h-full`}>
                <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 p-4 w-full max-w-md max-h-full">
                    {/* <!-- Modal content --> */}
                    <div className={`relative bg-white rounded-lg shadow ${mode==='dark' && "dark:bg-gray-700"}`}>
                        {/* <!-- Modal header --> */}
                        <div className={`flex items-center justify-between p-4 md:p-5 border-b rounded-t ${mode==='dark' && "dark:border-gray-600"}`}>
                            <h3 className={`text-xl font-semibold text-gray-900 ${mode==='dark' && "dark:text-white"}`}>
                                Add a budget
                            </h3>
                            <button onClick={() => {
                                setAddToggle((addToggle) => !addToggle)
                                setBName('')
                                }} type="button" className={`end-2.5 text-gray-400 bg-transparent hover:bg-gray-200 hover:text-gray-900 rounded-lg text-sm w-8 h-8 ms-auto inline-flex justify-center items-center ${mode==='dark' && "dark:hover:bg-gray-600 dark:hover:text-white"}`} data-modal-hide="authentication-modal">
                        ...
                        <div className="p-4 md:p-5">
                            <form className="space-y-4" onSubmit={handleSubmit}>
                                <div>
                                    <label htmlFor="name" className={`block mb-2 text-md text-gray-900 ${mode==='dark' && "dark:text-white"} font-light`}>Specify the name of the budget</label>
                                    <input 
                                    value={bName}
                                    ...
                                    name="name" 
                                    id="name" 
                                    className={`bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 ${mode==='dark' && "dark:bg-gray-600 dark:border-gray-500 dark:placeholder-gray-400 dark:text-white"}`}
                                    required
                                    />
                                </div>
