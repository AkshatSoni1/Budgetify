...
const TotalCard = (props) => {
  const { name } = props;
  const { user, month, year, TViewToggle,totalAmount, maximum,currentMonth, setTViewToggle, setTotalToggle, mode} = useContext(AppContext)
...
const handleViewUpdation = async() => {
  ...
  return (
    ...
    <div className={`border  px-6 py-4 m-4 rounded-md ${percentage >= 75 ? "bg-red-200 border-red-400" : percentage<50 ? "bg-blue-200 border-blue-400" :"bg-yellow-100 border-yellow-400"} ${mode === 'light' && "shadow-md shadow-gray-400"} z-40`}>
